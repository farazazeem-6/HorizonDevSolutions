...
const SPECIALTIES = [
  'WordPress', 'WooCommerce', 'Custom Websites',
...
export default function AboutSection() {
  ...
  return (
    <section id="about" className="bg-black py-14 sm:py-20 md:py-28 px-3 sm:px-4 md:px-6">
      <div className="max-w-6xl mx-auto">
        <div className="bg-[#101010] rounded-2xl md:rounded-3xl p-5 sm:p-8 md:p-12 lg:p-16">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-20">
            ...
            <div>
              <div className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl leading-[0.95] mb-6 sm:mb-8">
                <WordsPullUpMultiStyle
                  segments={[
                    { text: 'We build things that', className: 'font-normal text-primary' },
                    { text: 'actually work.', className: 'font-serif italic text-primary' },
                  ]}
            ...
            {/* Right — stats grid */}
            <div ref={statsRef} className="grid grid-cols-2 gap-3 sm:gap-4 content-start">
              {STATS.map((stat, i) => (
                <motion.div
                  key={stat.label}
                ...
                >
                  <p className="text-3xl sm:text-4xl md:text-5xl font-bold tracking-tight mb-1" style={{ color: '#E1E0CC' }}>
                    {stat.value}
                  </p>
                  <p className="text-gray-500 text-[10px] sm:text-xs leading-snug">{stat.label}</p>
                </motion.div>
              ))}
            </div>
          </div>
